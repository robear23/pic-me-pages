...
// Calculate content area with gutter for binding side
export function getContentArea(bindingType: 'SADDLE' | 'COIL') {
  const gutter = bindingType === 'COIL' ? LULU_CONFIG.GUTTER_COIL : LULU_CONFIG.GUTTER_SADDLE;
  
  return {
    left: LULU_CONFIG.CONTENT_LEFT + gutter,  // Add gutter to left (binding side)
    right: LULU_CONFIG.CONTENT_RIGHT,
    top: LULU_CONFIG.CONTENT_TOP,
    bottom: LULU_CONFIG.CONTENT_BOTTOM,
    width: LULU_CONFIG.CONTENT_RIGHT - (LULU_CONFIG.CONTENT_LEFT + gutter),
    height: LULU_CONFIG.CONTENT_BOTTOM - LULU_CONFIG.CONTENT_TOP,
  };
}
