...
// Module-level variables so the SIGTERM handler can reset the active job
let _activeJobId: string | null = null;
let _activeSupabaseUrl: string | null = null;
let _activeServiceKey: string | null = null;

// Catch process termination signals — Supabase kills the function after its execution limit.
// When that happens, mark the job as pending so the stall detection can re-queue it.
Deno.addSignalListener("SIGTERM", async () => {
  console.error("🛑 SIGTERM received - edge function being terminated");
  if (_activeJobId && _activeSupabaseUrl && _activeServiceKey) {
    try {
      const sb = createClient(_activeSupabaseUrl, _activeServiceKey);
      await sb.from('book_generation_jobs').update({
        status: 'pending',
        started_at: null,
        last_heartbeat: null,
        error_message: 'Function terminated by Supabase — will retry',
        updated_at: new Date().toISOString(),
      }).eq('id', _activeJobId).eq('status', 'processing');
      console.error(`🔄 Job ${_activeJobId} reset to pending after SIGTERM`);
    } catch (e) {
      console.error('Failed to reset job on SIGTERM:', e);
    }
  }
});
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Store for SIGTERM handler
    _activeSupabaseUrl = supabaseUrl;
    _activeServiceKey = supabaseServiceKey;
    ...
    _activeJobId = job.id; // Allow SIGTERM handler to reset this job if function is killed

    // PHASE 2: Check if book is already in terminal state before processing
    // EXCEPTION: rework/fix-missing-pages jobs must bypass this check and reset the book
    const isReworkJob = job.generation_data?.isReworkMode === true || job.generation_data?.isFixingMissingPages === true;
    if (job.book_id) {
      ...
      if (existingBook?.status === 'completed' || existingBook?.status === 'failed') {
        if (isReworkJob) {
          // Fix/rework jobs are intentional retries — reset the book to generating and continue
          console.log(`🔄 Book ${job.book_id} is ${existingBook.status} but this is a rework/fix job — resetting to generating`);
          await supabase
            .from('books')
    ...
    try {
      ...
      // TIMEOUT: Reset to pending so the queue worker resumes from saved pages.
      // Never permanently fail a job just because the edge function ran out of time.
      const isTimeoutError = errorMessage.includes('timeout') || errorMessage.includes('Timeout') ||
        errorMessage.includes('Edge function timeout');

      if (isTimeoutError) {
        console.log(`⏸️ Job ${job.id} timed out — resetting to pending so it can resume`);
        await supabase
          .from('book_generation_jobs')
          .update({
            status: 'pending',
            started_at: null,
            last_heartbeat: null,
            error_message: `Timed out, will resume automatically: ${errorMessage}`,
            updated_at: new Date().toISOString(),
          })
          .eq('id', job.id);
        // Do NOT mark book as failed — progress was saved incrementally
        if (stopHeartbeat) stopHeartbeat();
        return new Response(JSON.stringify({ message: 'Job timed out - will resume', jobId: job.id }), {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }

      // RATE_LIMITED: re-queue so the scheduler retries after the quota window resets.
      // Safe because the attempts counter (max 10) prevents infinite loops.
      const isRateLimitedError = errorMessage.includes('RATE_LIMITED');
      if (isRateLimitedError) {
        console.log(`⏳ Job ${job.id} rate limited — resetting to pending for automatic retry`);
        await supabase
          .from('book_generation_jobs')
          .update({
            status: 'pending',
            started_at: null,
            last_heartbeat: null,
            error_message: `Rate limited, will retry automatically: ${errorMessage}`,
            updated_at: new Date().toISOString(),
          })
          .eq('id', job.id);
        if (stopHeartbeat) stopHeartbeat();
        return new Response(JSON.stringify({ message: 'Job rate limited - will retry', jobId: job.id }), {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }

      // Determine if this is a system error (grant retry credit)
      // DO NOT grant retries for user errors (credits depleted)
      ...
      _activeJobId = null; // Clear so SIGTERM handler doesn't reset an already-finished job
    }
...
async function processBookGeneration(supabase: any, job: GenerationJob, startTime: number, stopHeartbeat: (() => void) | null): Promise<Response> {
  ...
  try {
    ...
    // For rework mode, pull prompts from the existing book's pages so we never call generate-prompts
    if (isReworkMode && job.book_id && !cachedPrompts) {
      const { data: existingBook } = await supabase
        .from('books')
        .select('pages')
        .eq('id', job.book_id)
      ...
      if (existingBook?.pages && Array.isArray(existingBook.pages) && existingBook.pages.length > 0) {
        const bookPrompts = existingBook.pages
          .sort((a: any, b: any) => (a.pageNumber ?? a.page_number ?? 0) - (b.pageNumber ?? b.page_number ?? 0))
          .map((p: any) => p.prompt)
          .filter(Boolean);

        if (bookPrompts.length > 0) {
          console.log(`📝 Rework mode: using ${bookPrompts.length} existing prompts from book pages`);
          prompts = bookPrompts;
        }
      }
    }

    if (!prompts && cachedPrompts && Array.isArray(cachedPrompts) && cachedPrompts.length > 0) {
    ...
    if (!prompts) {
      ...
      try {
        ...
        if (!promptsFetchResponse.ok) {
          ...
          } else if (promptsFetchResponse.status >= 500) {
            // Transient Gemini outage — re-queue so the scheduler retries automatically
            throw new Error(`RATE_LIMITED: AI service temporarily unavailable (${promptsFetchResponse.status}). Will retry automatically.`);
          } else {
...
Write ONLY the description, nothing else.`
    ...
    // Dynamic memory-based circuit breaker and time-based batching
    ...
    const MAX_PAGES_PER_BATCH = isReworkMode ? 10 : 3; // Hard limit to prevent 15-min Edge function timeouts
    ...
    for (const pageIndex of pagesToProcess) {
      ...
      // Dynamic circuit breaker - check memory or max batch limit
      if ((processedCount >= MIN_PAGES_BEFORE_CHECK && percentUsed > PAUSE_MEMORY_THRESHOLD) || processedCount >= MAX_PAGES_PER_BATCH) {
        console.log(`💤 Graceful pause: Processed ${processedCount} pages (Max: ${MAX_PAGES_PER_BATCH}), Memory: ${percentUsed.toFixed(1)}%`);
        ...
        await updateJobProgress(supabase, job.id, {
          currentStep: 'pausing_for_batch_or_memory_cleanup',
          currentPage: savedPageCount, // FIX #1: Use savedPageCount
          totalPages: adjustedPageCount
        });
        ...
        return new Response(
          JSON.stringify({ message: 'Batch complete, continuing generation', pages_completed: savedPageCount }), // FIX #1: Use savedPageCount
          { headers: corsHeaders }
        );
      }
      ...
      for (let attempt = 1; attempt <= MAX_PAGE_ATTEMPTS && !pageSuccess; attempt++) {
        try {
          ...
          // Use direct HTTP so the timeout actually cancels the connection (supabase.functions.invoke cannot be aborted)
          const generateImagesUrl = `${Deno.env.get('SUPABASE_URL')}/functions/v1/generate-images`;
          const httpResponse = await fetchWithTimeout(generateImagesUrl, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
          ...
          if (!httpResponse.ok) {
            ...
            console.error(`  Page ${pageIndex + 1} attempt ${attempt} HTTP error: ${httpResponse.status} - ${errText.substring(0, 200)}`);
            if (attempt < MAX_PAGE_ATTEMPTS) {
              await new Promise(resolve => setTimeout(resolve, 2000));
              continue;
            }
            throw new Error(`generate-images HTTP ${httpResponse.status}: ${errText.substring(0, 200)}`);
          }

          const imageResponse = await httpResponse.json();
          ...
          if (consecutiveFailures >= MAX_CONSECUTIVE_PAGE_FAILURES) {
            ...
            console.error(`🛑 ${consecutiveFailures} consecutive page failures - last error: ${errorMsg}`);

            // If failures are timeout-related, pause and let the queue resume rather than permanently failing
            const isTimeoutFailure = errorMsg.includes('timeout') || errorMsg.includes('Timeout');
            if (isTimeoutFailure) {
              console.log(`⏸️ Consecutive timeouts detected - pausing job for resume`);
              if (stopHeartbeat) stopHeartbeat();
              await supabase
                .from('book_generation_jobs')
                .update({
                  ...
                  started_at: null,
                  last_heartbeat: null,
                  error_message: `Page generation timing out (model slow) - will resume automatically`,
                  updated_at: new Date().toISOString(),
                })
                .eq('id', job.id);
              return new Response(
                JSON.stringify({ message: 'Paused due to timeouts - will resume', jobId: job.id }),
                { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
              );
            }

            // Non-timeout consecutive failures = actual model/content issue → fail permanently
            await supabase
              .from('book_generation_jobs')
              .update({
                status: 'failed',
    ...
    // Save essential data BEFORE clearing (prompts.length will be 0 after cleanup)
    const firstPageUrl = generatedPages[0]?.imageUrl;
    const totalExpected = prompts.length; // MUST capture before clearing
...
Minimal or no decorative elements. Clean, professional, ready for text overlay if needed.`;
    ...
    // (totalExpected was captured before memory cleanup — prompts.length is 0 by this point)
